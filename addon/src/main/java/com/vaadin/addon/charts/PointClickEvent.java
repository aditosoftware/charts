...
public class PointClickEvent extends AbstractPointEvent {
   ...
   * @param pointIndex
   */
  public PointClickEvent(
      Chart source, MouseEventDetails details, Series series, String category, int pointIndex) {
   ...
   *
   * <p>Note, that if the axis type is Date, the value is "unix timestamp" which is shifted to UTF
   * time zone that is used by the client side implementation. If you have used Date object as
   * value, you most likely want to pass the value thru {@link Util#toServerDate(double)} method
   * before actually using the value.
   *
   * <p>This value might be zero in a chart without axes like {@link ChartType#PIE} or {@link
   * ChartType#TREEMAP}
   *
