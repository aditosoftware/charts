...
public class ChartDesignWriter implements Serializable {
  ...
  /**
   * These properties are ignored when reading values from configuration object For example,
   * serialVersionUid is not wanted to be added to declarative html
   */
  private static final List<String> ignoredConfigurationObjectProperties =
      Arrays.asList("serialversionuid", "series", "changelisteners", "axisindex", "configuration");

  private static final HashMap<
          Class<? extends AbstractConfigurationObject>, AbstractConfigurationObject>
      cache =
  ...
  public static void writeConfigurationToElement(
    ...
    for (Field field : declaredFields) {
      if (ignoredConfigurationObjectProperties.contains(field.getName().toLowerCase())) {
        continue;
      }
      ...
      if (value != null) {
        if (isPlotOptions(field)) {
          writePlotOptions(parent, (Map<String, AbstractPlotOptions>) value, context);
          ...
          createElementForConfigurationObject(
              (AbstractConfigurationObject) value, parent, field.getName(), context);
        } else if (isCollection(field)) {
          writeCollection(configuration, parent, field, (Collection) value, context);
  ...
  private static void writeAttribute(
    ...
    ChartDesignAttributeHandler.writeAttribute(
        configuration, attributeName, parent.attributes(), defaultConfiguration, context);
    if (ChartDesignCommon.isReservedProperty(attributeName)
      ...
      parent
          ...
          .put(
              new Attribute(
                  ChartDesignCommon.RESERVED_PROPERTY_PREFIX + attributeName, attributeValue));
    }
  }
  ...
  private static void writePlotOptions(
      Element parent, Map<String, AbstractPlotOptions> plotOptions, DesignContext context) {
    ...
    for (Map.Entry<String, AbstractPlotOptions> entry : plotOptions.entrySet()) {
      createElementForConfigurationObject(
          entry.getValue(), plotOptionsElement, entry.getKey(), context);
    }
  }
  ...
  private static Object readFieldValue(AbstractConfigurationObject configuration, Field field) {
    ...
    try {
      ...
      logger.log(
          Level.WARNING,
          "Could not get value from field "
              + field.getName()
              + " in "
              + configuration.getClass().getName());
    }
  ...
  private static void createElementForConfigurationObject(
      AbstractConfigurationObject newConfiguration,
      Element parent,
      String tagName,
      DesignContext context) {
  ...
  private static AbstractConfigurationObject readDefaultFromChartConfigurationObject(
    ...
    for (Method method : Configuration.class.getMethods()) {
      if (method.getName().startsWith("get")
        ...
        try {
          return (AbstractConfigurationObject) method.invoke(defaultConfiguration);
        } catch (Exception e) {
          throw new DesignException(
              "Was not able to create default "
                  + clazz.getName()
                  + " from default Configuration object",
              e);
        }
      }
    }
  ...
  /**
   * Reads strings in camelcase format and returns node names where words are all lowercase and
   * splitted with character '-' For example, YAxis -> y-axis and dataLabels -> data-labels
   *
   * @param camelcase
   * @return node name
   */
