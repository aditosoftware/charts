...
/**
 * Modifier that adds special handling for {@link ChartOptions} and {@link GradientColor} inside
 * {@link ChartOptions}
 */
public class ChartOptionsBeanSerializerModifier extends DefaultBeanSerializerModifier {
  ...
  public List<BeanPropertyWriter> changeProperties(
    ...
    if (ChartOptions.class.isAssignableFrom(beanDesc.getBeanClass())) {
      ...
      for (BeanPropertyWriter writer : beanProperties) {
        if ("lang" == writer.getName()
            || "theme" == writer.getName()
            || "global" == writer.getName()) {
          newProperties.add(writer);
        }
      }
      return newProperties;
    } else if (GradientColor.class.isAssignableFrom(beanDesc.getBeanClass())) {
      // don't serialize gradients normally, instead use the custom
      // serializer
      List<BeanPropertyWriter> newProperties = new ArrayList<BeanPropertyWriter>();

      for (BeanPropertyWriter writer : beanProperties) {
        if ("radialGradient" != writer.getName() && "linearGradient" != writer.getName()) {
          newProperties.add(writer);
        }
      }
      return newProperties;
  ...
  public JsonSerializer<?> modifySerializer(
    ...
    if (GradientColor.class.isAssignableFrom(beanDesc.getBeanClass())) {
      return new ThemeGradientColorBeanSerializer((BeanSerializerBase) serializer);
    } else {
