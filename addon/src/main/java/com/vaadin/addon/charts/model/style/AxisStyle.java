...
public class AxisStyle extends AbstractConfigurationObject {
  ...
  /**
   * Sets the tick interval in scale units for the minor ticks. On a linear axis, if {@link
   * TickIntervalStyle#AUTO}, the minor tick interval is calculated as a fifth of the tickInterval.
   * If null, minor ticks are not shown.
   *
   * <p>On logarithmic axes, the unit is the power of the value. For example, setting the minor tick
   * interval to 1 puts one tick on each of 0.1, 1, 10, 100 etc. Setting the minor tick interval to
   * 0.1 produces 9 ticks between 1 and 10, 10 and 100 etc. A minor tick interval of {@link
   * TickIntervalStyle#AUTO} on a log axis results in a best guess, attempting to enter
  ...
  /**
   * Sets the alternate grid color. When using an alternate grid color, a band is painted across the
   * plot area between every other grid line. Defaults to null.
   *
  ...
  /**
   * Sets the color of the grid lines extending the ticks across the plot area. Defaults to
   * "#C0C0C0".
   *
