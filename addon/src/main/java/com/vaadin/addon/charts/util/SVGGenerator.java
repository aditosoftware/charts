...
/**
 * This class can be used to render a Chart displayed on the browser of the client as SVG. SVG is a
 * well supported format and it can be transferred to virtually any graphic format.
 *
 * <p>The implementation uses PhantomJS to render the chart on server side. Install version 1.9 or
 * newer separately from <a href="http://phantomjs.org/">phantomjs.org</a>. After installation
 * either ensure that phantomjs binary is available on PATH or set "phantom.exec" system property to
 * point into it.
 */
public class SVGGenerator {
  ...
  /**
   * Charset for communication with PhantomJS should always be UTF-8 and not default charset.
   *
   * <p>PhantomJS default console output encoding is UTF-8 http://goo.gl/aklNQa
   */
  ...
  /**
   * Creates in new {@link SVGGenerator} instance. The preferred way to get an instance is to use
  ...
  /**
   * @param theme The theme that will be used to create the SVG graphics. The default is the theme
   *     value in {@link ChartOptions#get()} object.
   * @return
   */
  ...
  /**
   * @param lang The lang options that will be used to create the SVG graphics. The default is the
   *     lang value in {@link ChartOptions#get()} object.
   * @return
   */
  ...
  /**
   * @param width The target width in pixels for the the chart. The default value is -1.
   * @return
   */
  ...
  /**
   * @param height The target height in pixels for the chart. The default value is -1.
   * @return
   */
  ...
  /**
   * @param timeline The boolean if timeline should be visible in the chart. The default value is
   *     false;
   * @return
   */
   ...
   *
   * @param conf the configuration that will be plotted as an SVG graphics
   ...
   * Generates an SVG file using given JSON configuration object.
   *
   * @param options the json options string that will be plotted as an SVG graphics
   * @return String containing SVG graphics
   * @see SVGGenerator
   * @see #withLang(String)
  ...
  public synchronized String generate(String options) {
    ...
    try {
      ...
      BufferedReader reader =
          new BufferedReader(new InputStreamReader(phantomJSProcess.getInputStream(), charset));
  ...
  /**
   * Ensure that the temporary files still exist and re-generate if they have been cleaned away from
   * /tmp
   */
    ...
    try {
      ...
      for (String string : scripts) {
        InputStream resourceAsStream =
            Chart.class.getResourceAsStream("/com/vaadin/addon/charts/client/" + string);
        copy(resourceAsStream, out);
        resourceAsStream.close();
      }
      InputStream resourceAsStream =
          SVGGenerator.class.getResourceAsStream("vaadin-charts-formatter.js");
      copy(resourceAsStream, out);
      resourceAsStream.close();
      ...
      resourceAsStream = SVGGenerator.class.getResourceAsStream("phantomconverter.js");
      copy(resourceAsStream, out);
      resourceAsStream.close();
      out.close();
    } catch (IOException e) {
