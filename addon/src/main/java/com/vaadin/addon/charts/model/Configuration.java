...
/**
 * Chart's configuration root object containing all the child objects that are used to configure
 * chart, axes, legend, titles etc.
 */
public class Configuration extends AbstractConfigurationObject implements ChartConfiguration {
  ...
  /**
   * Sets the actual series to append to the chart. Series objects contains the data content
   * (individual points/columns etc.) of the plot. The series object could have data (the content),
   * name, labels, tooltips, markers etc. <br>
   * <br>
   * In addition to the attributes listed above, any member of the plotOptions for that specific
   * type of plot can be added to a series individually. For example, even though a general
   * lineWidth is specified in AbstractPlotOptions, an individual lineWidth can be specified for
   * each series (e.g. to enable each series have different lineWidth).
   *
   * @param series
   */
  ...
  /**
   * If series is a {@link DataSeries} and has drilldown Series that haven't be added to the {@link
   * Drilldown} configuration object they will be added at this point
   *
   * @param series
   */
  ...
  /**
   * Configuration options for drill down, the concept of inspecting increasingly high resolution
   * data through clicking on chart items like columns or pie slices.
   *
  ...
  /**
   * Returns the X-axis or category axis. Normally this is the horizontal axis, though if the chart
   * is inverted this is the vertical axis. In case of multiple axes defined, the first axis is
   * returned. An axis will be created if no axis is defined.
   *
  ...
  /**
   * Returns the Y-axis or value axis. Normally this is the vertical axis, though if the chart is
   * inverted this is the horizontal axis. In case there are multiple axes defined (a list), only
   * the first axis is returned. If none exist, a Y-axis will be created.
   *
  ...
  /**
   * Returns the Z-axis. In case there are multiple axes defined (a list), only the first axis is
   * returned. If none exist, a Z-axis will be created.
   *
   ...
   * @return the tooltip options for this chart. If none have been set, a new Tooltip object with
   *     the default options is created.
   */
  ...
  /**
   * Sets the legend. The legend is a box containing a symbol and name for each series item or point
   * item in the chart.
   *
   * @param legend
   */
   ...
   *
   * <p>Returns null if no plot options was found for the type
   *
   * @see #setPlotOptions(AbstractPlotOptions)
   * @param type
   */
   ...
   *
   * <p>If the component contains several chart types, this method can be called with several plot
   * option types. Subsequent calls with the same plot type will replace previous options for that
   * specific chart type i.e only the latest options for each chart type is honored.
   *
   * <p>{@link PlotOptionsSeries} is a special plot options type that can be used to define rules
   * for all chart types.
   *
  ...
  /**
   * Adds a pane. This applies only to polar charts and angular gauges. This configuration object
   * holds general options for the combined X and Y -axes set. Each XAxis or YAxis can reference the
   * pane by index.
   *
   * @param pane
   */
   ...
   *
   * <p>This is only valid if the chart is configured to use timeline. See {@link
   * com.vaadin.addon.charts.Chart#setTimeline(boolean)}
   *
   * @param rangeSelector
   * @see RangeSelector
   */
   ...
   *
   * <p>Allows panning over the X axis of the chart
   *
   * @param scrollbar
   */
   ...
   *
   * <p>The actual text to display is set in the {@link Lang#setNoData(String)}
   *
   * @param noData
   */
  ...
  /**
   * Set loading options control the appearance of the loading screen that covers the plot area on
   * chart operations.
   *
   * @param loading
   */
  ...
  /**
   * Returns the {@link Lang} in use or {@code null} if no lang configuration has been set.
   *
  ...
  /**
   * Reverses the ListSeries (transposes it such that categories would be series names and vice
   * versa) to help stacking
   *
   * <p>throws {@link IllegalStateException} if series are not ListSeries type
   */
   ...
   *
   * @param animation Whether or not animation has been changed.
   */
   ...
   *
   * @param dimension Dimension of the axis.
   * @param axis Axis to get index for.
   ...
   *
   * @param axis Axis that is the source of the event.
   * @param minimum New minimum.
   * @param maximum New maximum.
   * @param redraw Whether or not to redraw.
   * @param animate Whether or not to animate.
   */
  @Override
  public void fireAxesRescaled(
      Axis axis, Number minimum, Number maximum, boolean redraw, boolean animate) {
    ...
    AxisRescaledEvent event =
        new AxisRescaledEvent(axisType, axisIndex, minimum, maximum, redraw, animate);
    for (ConfigurationChangeListener listener : changeListeners) {
   ...
   *
   * <p>This method is used internally by the library. Usage by the end user will cause unexpected
   * behavior.
   *
  ...
  /**
   * Returns the color axis. This is used in color-based diagrams, like heat maps. In case of
   * multiple axes defined, the first axis is returned. An axis will be created if no axis is
   * defined.
   *
  ...
  /** @return The Color-axis with the given index or null if the index is not valid */
   ...
   *
   * @param axis The color Axis to add.
   * @see #getColorAxis()
   */
