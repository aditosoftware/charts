 ...
 *
 * <p>The class uses {@link DataSeriesItem} to represent individual data points. The class also has
 * various helper methods and constructors that allow passing data as arrays or lists.
 *
 * @see ListSeries
 * @see RangeSeries
 * @see HeatSeries
...
public class DataSeries extends AbstractSeries {
  ...
  @JsonIgnore private List<Series> drilldownSeries = new ArrayList<Series>();

  /**
   * Constructs an empty {@link DataSeries}. Developers should then populate the series with various
   * addData and setData methods.
   */
  public DataSeries() {}
   ...
   *
   * @param items items to be contained in the constructed DataSeries
   */
   ...
   *
   * <p>e.g. <code>[[0, 15], [10, -50], [20, -56.5]...</code> could be inserted as follows <code>
   * new Number[][] { { 0, 15 }, { 10, -50 }, { 20, -56.5 }</code>
   *
   * @param entries An array of Numbers representing the (x,y) data pairs.
   */
  ...
  /**
   * Sets the data entries, first clearing the old ones. Uses the given category names and numeric
   * values.
   *
   * <p>The categoryNames and values arrays must be of equal length.
   *
   * @param categoryNames An array of the category names.
   * @param values An array of the values for each category in the categoryNames parameter.
   */
  ...
  /**
   * Sets the data entries, first clearing the old ones. Uses the given category names, numeric
   * values, and colors.
   *
   * <p>The categoryNames, values and colors arrays must be of equal length.
   *
   * @param categoryNames An array of the category names.
   * @param values An array of the values for each category in the categoryNames parameter.
   * @param colors An array of colors for each category name, value pair.
   */
  ...
  /**
   * Sets the data entries, first clearing the old ones. Uses the same numeric value for names
   * (value.toString) and Y-values.
   *
   * @param values
   */
  ...
  /**
   * @param name The name of the data item to find.
   * @return The first {@link DataSeriesItem} identified by the specified name. Returns null if no
  ...
  /**
   * Adds a data item and immediately updates the chart if it already has been drawn. If the chart
   * has not yet been drawn, all items added will be added to the chart when the chart is drawn.
   *
  ...
  /**
   * Adds a new item to the series data. And sets the series as its drilldown. Used for eager
   * loading drilldown. Series must have an id.
   *
   * <p>The remaining drilldown configurations can be set in {@link Configuration#getDrilldown()}
   *
   * @param item
   * @param series
   */
  ...
  /**
   * Adds a new item to the series data and enables drilldown for it. Used for lazy loading
   * drilldown. Using async drilldown requires setting {@link
   * Chart#setDrilldownCallback(com.vaadin.addon.charts.DrilldownCallbackHandler)} to return a
  ...
  /**
   * Adds a data item and immediately sends an update to the chart if so specified. Immediately
   * updating the chart causes it to dynamically add the data point.
   *
   * <p>This method is useful if you want to add many items without a client/server round-trip for
   * each item added. Do this by setting the updateChartImmediately parameter to false.
   *
   * @param item The item to add.
   * @param updateChartImmediately Updates the chart immediately if true.
   * @param shift If true, the first item from the series is removed. Handy if dynamically adjusting
   *     adding points and fixed amount of points should be kept visible.
   */
  ...
  /**
   * Triggers an update of the chart for the specified data item. Only the Y value of the
   * DataSeriesItem is updated.
   *
   ...
   * @param index
   * @return the Item
   * @throws IndexOutOfBoundsException if data series don't have item at given index
   */
   ...
   * @param sliced When true, the point is sliced out. When false, the point is set in. When null
   *     the sliced state is toggled
   */
   ...
   * @param sliced When true, the point is sliced out. When false, the point is set in. When null
   *     the sliced state is toggled
   * @param redraw Whether to redraw the chart after the point is altered.
   */
   ...
   *     the sliced state is toggled
   * @param redraw Whether to redraw the chart after the point is altered.
   * @param animation When true, the move will be animated with default animation options
   */
  public void setItemSliced(int index, boolean sliced, boolean redraw, boolean animation) {
    ...
    getConfiguration().fireItemSliced(this, index, sliced, redraw, animation);
  }
}
