...
public class Tooltip extends AbstractConfigurationObject {
  ...
  /**
   * Enable or disable animation of the tooltip. In slow legacy IE browsers the animation is
   * disabled by default.
   *
   * <p>Defaults to: true
   */
   ...
   *
   * <p>Defaults to: rgba(255, 255, 255, 0.85)
   */
   ...
   * guessed based on the closest data points. This member gives the default string representations
   * used for each unit. For an overview of the replacement codes, see <a
   ...
   * by override in the <code>plotOptions</code> for those series types.
   *
   * <p>For touch moves to behave the same way, <a
   * href="#tooltip.followTouchMove">followTouchMove</a> must be <code>true</code> also.
   *
   * <p>Defaults to: false
   */
  ...
  /**
   * Whether the tooltip should follow the finger as it moves on a touch device. If <a
   * href="#chart.zoomType">chart.zoomType</a> is set, it will override <code>followTouchMove</code>
   * .
   *
   * <p>Defaults to: true
   */
   ...
   * variables are <code>point.key</code>, <code>series.name</code>, <code>series.color</code> and
   * other members from the <code>point</code> and <code>series</code> objects. The <code>point.key
   * </code> variable contains the category name, x value or datetime string depending on the type
   * of axis. For datetime axes, the <code>point.key</code> date format can be set using
   * tooltip.xDateFormat.
   *
   * <p>Defaults to <code>&lt;span style="font-size: 10px"&gt;{point.key}&lt;/span&gt;&lt;br/&gt;
   * </code>
   */
   ...
   * Available variables are point.x, point.y, series.name and series.color and other properties on
   * the same form. Furthermore, point.y can be extended by the <code>tooltip.valuePrefix</code> and
   * <code>tooltip.valueSuffix</code> variables. This can also be overridden for each series, which
   * makes it a good hook for displaying units.
   *
   * <p>Defaults to: <span style="color:{point.color}">\u25CF</span> {series.name}: <b>{point.y}</b>
   * <br>
   */
  ...
  /**
   * The name of a symbol to use for the border around the tooltip. In Highcharts 3.x and less, the
   * shape was <code>square</code>.
   *
   * <p>Defaults to: callout
   */
   ...
   * Tooltip texts for series types with ordered data (not pie, scatter, flags etc) will be shown in
   * a single bubble. This is recommended for single series charts and for tablet/mobile optimized
   * charts.
   *
   * <p>See also the experimental implementation for <a href=
   * "http://jsfiddle.net/gh/get/jquery/1.7.2/highcharts/highcharts/tree/master/samples/highcharts/studies/tooltip-split/"
   * >tooltip.split</a>, that is better suited for charts with many series, especially line-type
   * series.
   *
   * <p>Defaults to: false
   */
  ...
  /**
   * Proximity snap for graphs or single points. Does not apply to bars, columns and pie slices. It
   * defaults to 10 for mouse-powered devices and 25 for touch devices. Note that since Highcharts
   * 4.1 the whole plot area by default captures pointer events in order to show the tooltip, so for
   * tooltip.snap to make sense, <a href="#plotOptions.series.stickyTracking">stickyTracking</a>
  ...
  /**
   * CSS styles for the tooltip. The tooltip can also be styled through the CSS class <code>
   * .highcharts-tooltip</code>.
   *
   * <p>Defaults to: { "color": "#333333", "cursor": "default", "fontSize": "12px", "padding":
   * "8px", "pointerEvents": "none", "whiteSpace": "nowrap" }
   */
   ...
   * it works around rtl bugs in early Firefox.
   *
   * <p>Defaults to: false
   */
  ...
  /**
   * How many decimals to show in each series' y value. This is overridable in each series' tooltip
  ...
  /**
   * How many decimals to show for the <code>point.change</code> value when the <code>series.compare
   * </code> option is set. This is overridable in each series' tooltip options object. The default
   * is to preserve all decimals.
   */
