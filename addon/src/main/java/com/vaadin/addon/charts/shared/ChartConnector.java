...
import com.vaadin.addon.charts.client.ui.*;
...
public class ChartConnector extends AbstractComponentConnector implements DeferredWorker {
  ...
  public ChartConnector() {
    ...
    registerRpc(
        ChartClientRpc.class,
        new ChartClientRpc() {
          @Override
          public void addPoint(final String pointJson, final int seriesIndex, final boolean shift) {
            Scheduler.get()
                .scheduleDeferred(
          ...
          public void updatePointValue(
              final int seriesIndex, final int pointIndex, final double newValue) {
            Scheduler.get()
                .scheduleDeferred(
                    new ScheduledCommand() {
                      @Override
                ...
                .scheduleDeferred(
                    new ScheduledCommand() {
                      ...
                      public void execute() {
                        ...
                        switch (axisCategory) {
                          case X_AXIS:
                            getWidget().updatexAxis(axisIndex, minimum, maximum, redraw, animate);
                            break;
                          case Y_AXIS:
                            getWidget().updateyAxis(axisIndex, minimum, maximum, redraw, animate);
                            break;
                          case Z_AXIS:
                            getWidget().updatezAxis(axisIndex, minimum, maximum, redraw, animate);
                            break;
                          case COLOR_AXIS:
                            getWidget()
                                .updateColorAxis(axisIndex, minimum, maximum, redraw, animate);
                            break;
                          default:
                            getWidget().updateyAxis(axisIndex, minimum, maximum, redraw, animate);
                            break;
                ...
                .scheduleDeferred(
                    new ScheduledCommand() {
                      ...
                      public void execute() {
                        ...
                        getWidget().slicePoint(seriesIndex, pointIndex, sliced, redraw, animation);
                      }
                    });
          }
  ...
  public void onStateChanged(StateChangeEvent stateChangeEvent) {
    ...
    final HighchartConfig cfg =
        HighchartConfig.createFromServerSideString(getState().confState, getState().jsonState);
    ...
    if (hasEventListener(CHART_CLICK_EVENT_ID)) {
      cfg.setClickHandler(
          new ChartClickHandler() {
            @Override
            public void onClick(ChartClickEvent event) {
              MouseEventDetails details =
                  MouseEventDetailsBuilder.buildMouseEventDetails(event, getWidget());
    ...
    if (hasEventListener(POINT_CLICK_EVENT_ID)) {
      cfg.setSeriesPointClickHandler(
          new PointClickHandler() {
    ...
    if (hasEventListener(CHART_SELECTION_EVENT_ID)) {
      cfg.setChartSelectionHandler(
          new ChartSelectionHandler() {
    ...
    cfg.setLegendItemClickHandler(
        new LegendItemClickHandler() {
            ...
            if (hasEventListener(LEGENDITEM_CLICK_EVENT_ID)) {
              ...
              MouseEventDetails mouseEventDetails =
                  MouseEventDetailsBuilder.buildMouseEventDetails(
                      event.getBrowserEvent(), getWidget());
              rpc.onLegendItemClick(seriesIndex, event.getSeriesItemIndex(), mouseEventDetails);
            }
    ...
    if (hasEventListener(CHECKBOX_CLICK_EVENT_ID)) {
      cfg.setCheckboxClickHandler(
          new CheckboxClickHandler() {
            @Override
    ...
    if (hasEventListener(HIDE_SERIES_EVENT_ID)) {
      cfg.setSeriesHideHandler(
          new SeriesHideHandler() {
            @Override
    ...
    if (hasEventListener(SHOW_SERIES_EVENT_ID)) {
      cfg.setSeriesShowHandler(
          new SeriesShowHandler() {
            @Override
    ...
    if (hasEventListener(X_AXES_EXTREMES_CHANGE_EVENT_ID)) {
      cfg.setXAxesAfterSetExtremeHandler(
          new AfterSetExtremeHandler() {
            @Override
    ...
    if (hasEventListener(Y_AXES_EXTREMES_CHANGE_EVENT_ID)) {
      cfg.setYAxesAfterSetExtremeHandler(
          new AfterSetExtremeHandler() {
            @Override
            public void afterSetExtreme(SetExtremesEvent event) {
    ...
    if (hasEventListener(POINT_SELECT_EVENT_ID)) {
      cfg.setPointSelectHandler(
          new PointSelectHandler() {
            @Override
    ...
    if (hasEventListener(POINT_UNSELECT_EVENT_ID)) {
      cfg.setPointUnselectHandler(
          new PointUnselectHandler() {
