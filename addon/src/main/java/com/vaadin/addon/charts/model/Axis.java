...
public abstract class Axis extends AbstractConfigurationObject {
  ...
  @JsonIgnore private ChartConfiguration configuration;
   ...
   * startOnTick option is true, the min value might be rounded down.
   *
   * @param min
   */
   ...
   * influenced by chart.alignTicks.
   *
   * @param max
   */
  ...
  /**
   * Sets the minimum and maximum of the axes after rendering has finished. If the startOnTick and
   * endOnTick options are true, the minimum and maximum values are rounded off to the nearest tick.
   * To prevent this, these options can be set to false before calling setExtremes.
   *
   ...
   * @param min Minimum.
   * @param max Maximum.
   * @param redraw Whether or not to redraw the chart.
   */
   ...
   * @param redraw Whether or not to redraw the chart.
   * @param animate Whether or not to animate the rescaling.
   */
  public void setExtremes(Number minimum, Number maximum, boolean redraw, boolean animate) {
    min = minimum;
    max = maximum;
    if (configuration != null) {
      configuration.fireAxesRescaled(this, minimum, maximum, redraw, animate);
    }
  }
  ...
  /**
   * Sets the configuration this axis is bound to. This method is automatically called by
   * configuration, when the axis is added to it.
   *
   * @param configuration Configuration this object is linked to.
   */
   ...
   * a web page, decimals must be avoided in the axis tick labels.
   */
  ...
  /**
   * If categories are present for the axis, names are used instead of numbers for that axis. Since
   * Highcharts 3.0, categories can also be extracted by giving each point a <code>name</code> and
   * setting axis type to <code>category</code>. However, if you have multiple series, best practice
   ...
   * gives the default string representations used for each unit. For an overview of the replacement
   ...
   * daily data, this can be used to prevent the axis from showing hours.
   */
   ...
   *
   * <p>On axes using <code>categories</code>, minor ticks are not supported.
   */
   ...
   * more axes are displayed on the same side of the plot.
   */
   ...
   * computed to approximately follow the <code>tickPixelInterval</code> on linear and datetime
   * axes. On categorized axes, a <code>null</code> tickInterval will default to 1, one category.
   * Note that datetime axes are based on milliseconds, so for example an interval of one day is
   * expressed as <code>24 * 3600 * 1000</code>.
   *
   ...
   * tick marks. Not applicable to categorized axis.
   */
  ...
  /**
   * The type of axis. Can be one of <code>"linear"</code>, <code>"logarithmic"</code>, <code>
   * "datetime"</code> or <code>"category"</code>. In a datetime axis, the numbers are given in
   * milliseconds, and tick marks are placed on appropriate values like full hours or days. In a
   * category axis, the point names of the chart's series are used for categories, if not a
