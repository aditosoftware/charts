 ...
 *
 * <p>All configuration and data is given through a {@link Configuration} object. Here is a simple
 * usage example:
 *
 * <p><code><pre>
 ...
 * </pre></code>
 *
 * <p>Unless otherwise documented, dynamic changes to the chart configuration are not immediately
 * reflected to an already drawn component. To redraw the chart you should call {@link #drawChart()}
 * or {@link #drawChart(Configuration)}.
 *
 * <p>See more examples in Book of Vaadin or the online demos.
 *
...
public class Chart extends AbstractComponent {
  ...
  /**
   * Forwards changes broadcasted by configurations into client RPC method calls.
   *
   * @since 2.0
   */
  private static class ProxyChangeForwarder implements ConfigurationChangeListener {
      ...
      if (event.getItem() != null) {
        if (event.getItem().getX() != null) {
          ...
          chart
              ...
              .addPoint(toJSON(event.getItem()), getSeriesIndex(event), event.isShift());
        }
      }
    }
    ...
    public void dataRemoved(DataRemovedEvent event) {
      chart.getRpcProxy(ChartClientRpc.class).removePoint(event.getIndex(), getSeriesIndex(event));
    }
    ...
    public void dataUpdated(DataUpdatedEvent event) {
      if (event.getValue() != null) {
        chart
            .getRpcProxy(ChartClientRpc.class)
            .updatePointValue(
                getSeriesIndex(event), event.getPointIndex(), event.getValue().doubleValue());
      } else {
        chart
            .getRpcProxy(ChartClientRpc.class)
            .updatePoint(getSeriesIndex(event), event.getPointIndex(), toJSON(event.getItem()));
      }
    }
    ...
    public void seriesStateChanged(SeriesStateEvent event) {
      chart
          .getRpcProxy(ChartClientRpc.class)
          .setSeriesEnabled(getSeriesIndex(event), event.isEnabled());
    }

    @Override
    public void animationChanged(boolean animation) {
      chart.getRpcProxy(ChartClientRpc.class).setAnimationEnabled(animation);
    }
    ...
    public void drilldownAdded(int seriesIndex, int pointIndex, Series series) {
      chart
          .getRpcProxy(ChartClientRpc.class)
          .addDrilldown(toJSON((AbstractConfigurationObject) series), seriesIndex, pointIndex);
    }
    ...
    public void seriesChanged(SeriesChangedEvent event) {
      chart
          .getRpcProxy(ChartClientRpc.class)
          .updateSeries(
              getSeriesIndex(event), toJSON((AbstractConfigurationObject) event.getSeries()));
    }
  }
  ...
  private final class ChartServerRpcImplementation implements ChartServerRpc {
    ...
    public void onChartClick(MouseEventDetails details) {
      final ChartClickEvent chartClickEvent = new ChartClickEvent(Chart.this, details);
    ...
    public void onSelection(
      ...
      final ChartSelectionEvent selectionEvent =
          new ChartSelectionEvent(Chart.this, selectionStart, selectionEnd, valueStart, valueEnd);
    ...
    @Override
    public void onXAxesExtremesChange(int axisIndex, double minimum, double maximum) {
      XAxis axis = getConfiguration().getxAxis(axisIndex);
      XAxesExtremesChangeEvent event =
          new XAxesExtremesChangeEvent(Chart.this, axis, minimum, maximum);
      fireEvent(event);
    }

    @Override
    public void onYAxesExtremesChange(int axisIndex, double minimum, double maximum) {
      YAxis axis = getConfiguration().getyAxis(axisIndex);
      YAxesExtremesChangeEvent event =
          new YAxesExtremesChangeEvent(Chart.this, axis, minimum, maximum);
      fireEvent(event);
    }
  ...
  private static final Method chartDrillupMethod =
      ReflectTools.findMethod(ChartDrillupListener.class, "onDrillup", ChartDrillupEvent.class);
  private static final Method pointClickMethod =
      ReflectTools.findMethod(PointClickListener.class, "onClick", PointClickEvent.class);
  ...
  private static final Method legendItemClickMethod =
      ReflectTools.findMethod(LegendItemClickListener.class, "onClick", LegendItemClickEvent.class);
  private static final Method checkboxClickMethod =
      ReflectTools.findMethod(CheckboxClickListener.class, "onClick", CheckboxClickEvent.class);
  private static final Method showSeriesMethod =
      ReflectTools.findMethod(SeriesShowListener.class, "onShow", SeriesShowEvent.class);
  private static final Method hideSeriesMethod =
      ReflectTools.findMethod(SeriesHideListener.class, "onHide", SeriesHideEvent.class);
  private static final Method xAxesExtremesChangeMethod =
      ReflectTools.findMethod(
          XAxesExtremesChangeListener.class,
          "onXAxesExtremesChange",
          XAxesExtremesChangeEvent.class);
  private static final Method yAxesExtremesChangeMethod =
      ReflectTools.findMethod(
          YAxesExtremesChangeListener.class,
          "onYAxesExtremesChange",
          YAxesExtremesChangeEvent.class);
  private static final Method pointSelectMethod =
      ReflectTools.findMethod(PointSelectListener.class, "onSelect", PointSelectEvent.class);
  private static final Method pointUnselectMethod =
      ReflectTools.findMethod(PointUnselectListener.class, "onUnselect", PointUnselectEvent.class);
  private ChartServerRpcImplementation srvRpcImpl;

  private static final List<ChartType> TIMELINE_NOT_SUPPORTED =
      Arrays.asList(
          ChartType.PIE,
          ChartType.GAUGE,
   ...
   *
   * <p>Default dimensions are 100% width and 400px height. @See {@link Chart}
   */
  public Chart() {
   ...
   *
   * <p>In charts with multiple series, the type can also be defined by setting series specific plot
   * options.
   *
   * @see #Chart()
  ...
  public void beforeClientResponse(boolean initial) {
    ...
    if (initial || stateDirty) {
      ...
      getState().confState = configuration == null ? null : toJSON(configuration);
  ...
  private void validateTimelineAndConfiguration() {
    if (getState().timeline) {
      ...
      if (TIMELINE_NOT_SUPPORTED.contains(type)) {
        throw new RuntimeException("Timeline is not supported for chart type '" + type + "'");
      }
    }
  }
  ...
  private ChartType resolveChartType() {
    if (configuration.getSeries().isEmpty()
        ...
        || configuration.getSeries().get(0).getPlotOptions() instanceof PlotOptionsSeries) {
  ...
  /**
   * Draws chart with the given configuration. The configuration is given in JSON configuration
   * format and it overrides settings done via the standard {@link Configuration} object.
   *
   * <p>Although using strictly typed Java API (via {@link #getConfiguration()} is highly
   * encouraged, using the low level JSON configuration may be handy in some occasions.
   *
   * <p>Note, that this configuration is evaluated as JavaScript and the config may contain
   * JavaScript functions. Thus developers should pay attention to data that is passed to this
   * function.
   *
   * <p>Note, that if further modifications are done to the configuration, the method must be called
   * again to redraw the UI.
   *
   * @see #getConfiguration()
   * @see #drawChart(Configuration)
   * @param jsonConfig the chart configuration as a JSON string
   */
   ...
   *
   * <p>Note, that calling this method on already displayed component don't necessary update it.
   * Developer should call {@link #drawChart()} or {@link #drawChart(String)} method to force re
   * draw.
   *
   * @see #drawChart(String)
   * @param jsonConf
   ...
   *
   * <p>Note, that calling this method on already displayed component don't necessary update it.
   * Developer should call {@link #drawChart()} or {@link #drawChart(Configuration)} method to force
   * re draw.
   *
   * @param configuration
   */
   ...
   *
   * <p>Note that if you modify the underlying {@link Series} directly the chart will automatically
   * be updated to reflect this unless explicitly told not to. The methods listed below can be used
   * as an example.
   *
  ...
  public Registration addChartClickListener(ChartClickListener listener) {
    this.addListener(
        ChartConnector.CHART_CLICK_EVENT_ID, ChartClickEvent.class, listener, chartClickMethod);

    return () ->
        removeListener(ChartConnector.CHART_CLICK_EVENT_ID, ChartClickEvent.class, listener);
  }
  ...
  /**
   * Adds chart drillup listener, which will be notified of clicks on the 'Back to previous series'
   * button.
   *
   * @param listener
   */
  public Registration addChartDrillupListener(ChartDrillupListener listener) {
    this.addListener(
        ChartConnector.CHART_DRILLUP_EVENT_ID,
        ChartDrillupEvent.class,
        listener,
        chartDrillupMethod);

    return () ->
        removeListener(ChartConnector.CHART_DRILLUP_EVENT_ID, ChartDrillupEvent.class, listener);
  }
  ...
  public void removeChartDrillupListener(ChartDrillupListener listener) {
    this.removeListener(ChartConnector.CHART_DRILLUP_EVENT_ID, ChartDrillupEvent.class, listener);
  }

  /**
   * Adds checkbox click listener, which will be notified when user has clicked a checkbox in the
   * legend
   *
   * @param listener
   */
  public Registration addCheckBoxClickListener(CheckboxClickListener listener) {
    this.addListener(
        ChartConnector.CHECKBOX_CLICK_EVENT_ID,
        CheckboxClickEvent.class,
        listener,
        checkboxClickMethod);

    return () ->
        removeListener(ChartConnector.CHECKBOX_CLICK_EVENT_ID, CheckboxClickEvent.class, listener);
  }
  ...
  public void removeCheckBoxClickListener(CheckboxClickListener listener) {
    this.removeListener(ChartConnector.CHECKBOX_CLICK_EVENT_ID, CheckboxClickEvent.class, listener);
  }
  ...
  /**
   * Sets the Chart drilldown handler that's responsible for returning the drilldown series for each
   * drilldown callback when doing async drilldown
   *
  ...
  /**
   * Adds a point click listener, which will be notified of clicks on the points, bars or columns in
   * the chart
   *
   * @param listener
   */
  public Registration addPointClickListener(PointClickListener listener) {
    this.addListener(
        ChartConnector.POINT_CLICK_EVENT_ID, PointClickEvent.class, listener, pointClickMethod);

    return () ->
        removeListener(ChartConnector.POINT_CLICK_EVENT_ID, PointClickEvent.class, listener);
  }
   ...
   * <br>
   *
   * <p>Note that if a chart selection listener is set, default action for selection is prevented.
   * Most commonly this means that client side zoom doesn't work and you are responsible for setting
   * the zoom, etc in the listener implementation.
   *
   * @param listener
   */
  public Registration addChartSelectionListener(ChartSelectionListener listener) {
    this.addListener(
        ChartConnector.CHART_SELECTION_EVENT_ID,
    ...
    return () ->
        removeListener(
            ChartConnector.CHART_SELECTION_EVENT_ID, ChartSelectionEvent.class, listener);
  }
   ...
   *
   * <p>Note that adding a legend item click listener also disabled the default behaviour to toggle
   * series visibility. If that is not desired, you can enable it again by calling
  ...
  public Registration addLegendItemClickListener(LegendItemClickListener listener) {
    this.addListener(
        ...
        listener,
        legendItemClickMethod);
    setSeriesVisibilityTogglingDisabled(true);

    return () ->
        removeListener(
            ChartConnector.LEGENDITEM_CLICK_EVENT_ID, LegendItemClickEvent.class, listener);
  }
  ...
  public Registration addSeriesHideListener(SeriesHideListener listener) {
    this.addListener(
        ChartConnector.HIDE_SERIES_EVENT_ID, SeriesHideEvent.class, listener, hideSeriesMethod);

    return () ->
        removeListener(ChartConnector.HIDE_SERIES_EVENT_ID, SeriesHideEvent.class, listener);
  }
  ...
  public void removeSeriesHideListener(SeriesHideListener listener) {
    this.removeListener(ChartConnector.HIDE_SERIES_EVENT_ID, SeriesHideEvent.class, listener);
  }
  ...
  public Registration addSeriesShowListener(SeriesShowListener listener) {
    this.addListener(
        ChartConnector.SHOW_SERIES_EVENT_ID, SeriesShowEvent.class, listener, showSeriesMethod);

    return () ->
        removeListener(ChartConnector.SHOW_SERIES_EVENT_ID, SeriesShowEvent.class, listener);
  }
  ...
  public void removeSeriesShowListener(SeriesShowListener listener) {
    this.removeListener(ChartConnector.SHOW_SERIES_EVENT_ID, SeriesShowEvent.class, listener);
  }

  /**
   * Adds a x axes extremes change listener, which will be notified when an x axis extremes are
   * changed.
   *
   * @param listener
   */
  public Registration addXAxesExtremesChangeListener(XAxesExtremesChangeListener listener) {
    this.addListener(
   ...
   */
  @Deprecated
  public void removeXAxesExtremesChangeListener(XAxesExtremesChangeListener listener) {
    this.removeListener(
        ChartConnector.X_AXES_EXTREMES_CHANGE_EVENT_ID, XAxesExtremesChangeEvent.class, listener);
  }

  /**
   * Adds a y axes extremes change listener, which will be notified when an y axis extremes are
   * changed.
   *
   * @param listener
   */
  public Registration addYAxesExtremesChangeListener(YAxesExtremesChangeListener listener) {
    this.addListener(
   ...
   */
  @Deprecated
  public void removeYAxesExtremesChangeListener(YAxesExtremesChangeListener listener) {
    this.removeListener(
        ChartConnector.Y_AXES_EXTREMES_CHANGE_EVENT_ID, YAxesExtremesChangeEvent.class, listener);
  }

  /**
   * Adds a point select listener, which will be notified when an data point is selected.
   *
   * @param listener
   */
  public Registration addPointSelectListener(PointSelectListener listener) {
    this.addListener(
        ChartConnector.POINT_SELECT_EVENT_ID, PointSelectEvent.class, listener, pointSelectMethod);

    return () ->
        removeListener(ChartConnector.POINT_SELECT_EVENT_ID, PointSelectEvent.class, listener);
  }
  ...
  public void removePointSelectListener(PointSelectListener listener) {
    this.removeListener(ChartConnector.POINT_SELECT_EVENT_ID, PointSelectEvent.class, listener);
  }

  /**
   * Adds a point unselect listener, which will be notified when an data point is unselected.
   *
   * @param listener
   */
  public Registration addPointUnselectListener(PointUnselectListener listener) {
    this.addListener(
        ChartConnector.POINT_UNSELECT_EVENT_ID,
        PointUnselectEvent.class,
        listener,
        pointUnselectMethod);

    return () ->
        removeListener(ChartConnector.POINT_UNSELECT_EVENT_ID, PointUnselectEvent.class, listener);
  }
  ...
  public void removePointUnselectListener(PointUnselectListener listener) {
    this.removeListener(ChartConnector.POINT_UNSELECT_EVENT_ID, PointUnselectEvent.class, listener);
  }
  ...
  /**
   * The series visibility is toggled by default if user clicks the series item in the legend.
   * Calling setSeriesVisibilityTogglingDisabled( <code>true</code>) will disable this behaviour.
   *
   * @param disabled
   */
  ...
  /**
   * Toggles the timeline feature in the chart. When timeline is set as <code>true</code> both
   * RangeSelector and Navigator are enabled.
   *
   * @param timeline
   */
