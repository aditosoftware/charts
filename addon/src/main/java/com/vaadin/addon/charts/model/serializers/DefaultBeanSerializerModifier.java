...
/**
 * Logic for altering the bean serialization process. Mainly used when serialization needs to be
 * customized with a bean serializer.
 */
  ...
  public JsonSerializer<?> modifySerializer(
    ...
    if (DataProviderSeries.class.isAssignableFrom(beanDesc.getBeanClass())) {
      return new BeanSerializerDelegator<DataProviderSeries>(
          (BeanSerializerBase) serializer, new DataProviderSeriesBeanSerializer());
    } else if (DataSeriesItem.class.isAssignableFrom(beanDesc.getBeanClass())) {
      return new BeanSerializerDelegator<DataSeriesItem>(
          (BeanSerializerBase) serializer, new DataSeriesItemBeanSerializer());
    ...
    } else if (AbstractSeries.class.isAssignableFrom(beanDesc.getBeanClass())) {
      return new BeanSerializerDelegator<AbstractSeries>(
          (BeanSerializerBase) serializer, new AbstractSeriesBeanSerializer());
    } else {
