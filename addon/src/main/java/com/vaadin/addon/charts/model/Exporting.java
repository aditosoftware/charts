 ...
 * href="http://www.highcharts.com/docs/export-module/export-module-overview" >the docs</a>.
 */
@Generated(
...
public class Exporting extends AbstractConfigurationObject {
  ...
  /**
   * Experimental setting to allow HTML inside the chart (added through the <code>useHTML</code>
   * options), directly in the exported image. This allows you to preserve complicated HTML
   ...
   * like Batik, does not support it. This also applies to downloaded SVG that you want to open in a
   * desktop client.
   *
   * <p>Defaults to: false
   */
  ...
  /**
   * Options for the export related buttons, print and export. In addition to the default buttons
   * listed here, custom buttons can be added. See <a
   * href="#navigation.buttonOptions">navigation.buttonOptions</a> for general options.
   */
  ...
  /**
   * Whether to enable the exporting module. Disabling the module will hide the context button, but
   * API methods will still be available.
   *
   * <p>Defaults to: true
   */
   ...
   * this width, it is resized. After printing or cancelled, it is restored. The default width makes
   * the chart fit into typical paper format. Note that this does not affect the chart when printing
   ...
   * href="#chart.width">chart.width</a> is set. The width exported raster image is then multiplied
   * by <a href="#exporting.scale">scale</a>.
   */
  ...
  /**
   * Default MIME type for exporting if <code>chart.exportChart()</code> is called without
   * specifying a <code>type</code> option. Possible values are <code>image/png</code>, <code>
   * image/jpeg</code>, <code>application/pdf</code> and <code>image/svg+xml</code>.
   *
   * <p>Defaults to: image/png
   */
   ...
   * points to Highchart's free web service.
   *
   * <p>Defaults to: http://export.highcharts.com
   */
  ...
  /**
   * The pixel width of charts exported to PNG or JPG. As of Highcharts 3.0, the default pixel width
   * is a function of the <a href="#chart.width">chart.width</a> or <a
