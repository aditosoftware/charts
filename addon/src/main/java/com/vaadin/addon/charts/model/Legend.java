...
public class Legend extends AbstractConfigurationObject {
  ...
  /**
   * The horizontal alignment of the legend box within the chart area. Valid values are <code>left
   * </code>, <code>center</code> and <code>right</code>.
   *
   * <p>In the case that the legend is aligned in a corner position, the <code>layout</code> option
   * will determine whether to place it above/below or on the side of the plot area.
   *
   * <p>Defaults to: center
   */
  ...
  /**
   * CSS styles for each legend item when the corresponding series or point is hidden. Only a subset
   * of CSS is supported, notably those options related to text. Properties are inherited from
   * <code>style</code> unless overridden here. Defaults to:
   *
   * <pre>
   * itemHiddenStyle: {
   * 		color: '#CCC'
  ...
  /**
   * CSS styles for each legend item in hover mode. Only a subset of CSS is supported, notably those
   * options related to text. Properties are inherited from <code>style</code> unless overridden
   * here. Defaults to:
   *
   * <pre>
   * itemHoverStyle: {
  ...
  /**
   * CSS styles for each legend item. Only a subset of CSS is supported, notably those options
   * related to text.
   *
   * <p>Defaults to: { "color": "#333333", "cursor": "pointer", "fontSize": "12px", "fontWeight":
   * "bold" }
   */
  ...
  /**
   * A <a href= "http://www.highcharts.com/docs/chart-concepts/labels-and-string-formatting" >format
   * string</a> for each legend label. Available variables relates to properties on the series, or
   * the point in case of pies.
   *
   * <p>Defaults to: {name}
   */
  ...
  /**
   * Line height for the legend items. Deprecated as of 2.1. Instead, the line height for each item
   * can be set using itemStyle.lineHeight, and the padding between items using itemMarginTop and
   * itemMarginBottom.
   *
   * <p>Defaults to: 16
   */
  ...
  /**
   * Options for the paging or navigation appearing when the legend is overflown. Navigation works
   * well on screen, but not in static exported images. One way of working around that is to <a
   * href=
  ...
  /**
   * Whether to reverse the order of the legend items compared to the order of the series or points
  ...
  /**
   * Whether to apply a drop shadow to the legend. A <code>backgroundColor</code> also needs to be
   * applied for this to take effect. Since 2.3 the shadow can be an object configuration containing
   * <code>color</code>, <code>offsetX</code>, <code>offsetY</code>, <code>opacity</code> and <code>
   * width</code>.
   *
   * <p>Defaults to: false
   */
  ...
  /**
   * The pixel height of the symbol for series types that use a rectangle in the legend. Defaults to
   * the font size of legend items.
   */
  ...
  /**
   * Whether to <a href=
   * "http://www.highcharts.com/docs/chart-concepts/labels-and-string-formatting#html" >use HTML</a>
   * to render the legend item texts. Prior to 4.1.7, when using HTML, <a
   * href="#legend.navigation">legend.navigation</a> was disabled.
   *
   * <p>Defaults to: false
   */
  ...
  /**
   * The vertical alignment of the legend box. Can be one of <code>top</code>, <code>middle</code>
   * or <code>bottom</code>. Vertical position can be further determined by the <code>y</code>
   * option.
   *
   * <p>In the case that the legend is aligned in a corner position, the <code>layout</code> option
   * will determine whether to place it above/below or on the side of the plot area.
   *
   * <p>Defaults to: bottom
   */
  ...
  /**
   * The x offset of the legend relative to its horizontal alignment <code>align</code> within
   * chart.spacingLeft and chart.spacingRight. Negative x moves it to the left, positive x moves it
   * to the right.
   *
   * <p>Defaults to: 0
   */
  ...
  /**
   * The vertical offset of the legend relative to it's vertical alignment <code>verticalAlign
   * </code> within chart.spacingTop and chart.spacingBottom. Negative y moves it up, positive y
   * moves it down.
   *
   * <p>Defaults to: 0
   */
