...
/**
 * The Y axis or value axis. In case of multiple axes, the yAxis node is an array of configuration
 * objects.
 *
 * <p>See <a class="internal" href="#axis.object">the Axis object</a> for programmatic access to the
 * axis.
 */
@Generated(
...
public class YAxis extends Axis {
  ...
  /**
   * Whether to allow decimals in this axis' ticks. When counting integers, like persons or hits on
   * a web page, decimals should be avoided in the labels.
   *
   * <p>Defaults to: true
   */
  ...
  /**
   * When using an alternate grid color, a band is painted across the plot area between every other
   * grid line.
   */
  ...
  /**
   * An array defining breaks in the axis, the sections defined will be left out and all the points
   * shifted closer to each other. Requires that the broken-axis.js module is loaded.
   */
  ...
  /**
   * If categories are present for the xAxis, names are used instead of numbers for that axis. Since
   * Highcharts 3.0, categories can also be extracted by giving each point a <a
   * href="#series.data">name</a> and setting axis <a href="#xAxis.type">type</a> to <code>category
   * </code>. However, if you have multiple series, best practice remains defining the <code>
   ...
   * units may be used, for example the <code>day</code> unit can be used on midnight and <code>hour
   * </code> unit be used for intermediate values on the same axis. For an overview of the
   * replacement codes, see <a href="#Highcharts.dateFormat">dateFormat</a>.
   *
   * <p>Defaults to:
   *
   * <pre>
  ...
  /**
   * The dash or dot style of the grid lines. For possible values, see <a href=
  ...
  /**
   * Polar charts only. Whether the grid lines should draw as a polygon with straight lines between
   * categories, or as circles. Can be either <code>circle</code> or <code>polygon</code>.
   *
   * <p>Defaults to: null
   */
  ...
  /**
   * Index of another axis that this axis is linked to. When an axis is linked to a master axis, it
   * will take the same extremes as the master, but as assigned by min or max or by setExtremes. It
   * can be used to show additional info, or to ease reading the chart by duplicating the scales.
   */
  ...
  /**
   * Solid gauge only. Unless <a href="#yAxis.stops">stops</a> are set, the color to represent the
   * maximum value of the Y axis.
   *
   * <p>Defaults to: #102D4C
   */
   ...
   * smaller interval than this. For example, for a datetime axis the main unit is milliseconds. If
   * minRange is set to 3600000, you can't zoom in more than to one hour.
   *
   * <p>The default minRange for the x axis is five times the smallest interval between any of the
   * data points.
   *
   * <p>On a logarithmic axis, the unit for the minimum range is the power. So a minRange of 1 means
   ...
   * <code>endOnTick</code> settings also affect how the extremes of the axis are computed.
   */
   ...
   * daily data, this can be used to prevent the axis from showing hours. Defaults to the closest
   * distance between two points on the axis.
   */
  ...
  /**
   * The dash or dot style of the minor grid lines. For possible values, see <a href=
  ...
  /**
   * Tick interval in scale units for the minor ticks. On a linear axis, if <code>"auto"</code>, the
   * minor tick interval is calculated as a fifth of the tickInterval. If <code>null</code>, minor
   * ticks are not shown.
   *
   * <p>On logarithmic axes, the unit is the power of the value. For example, setting the
   * minorTickInterval to 1 puts one tick on each of 0.1, 1, 10, 100 etc. Setting the
   * minorTickInterval to 0.1 produces 9 ticks between 1 and 10, 10 and 100 etc. A minorTickInterval
   * of "auto" on a log axis results in a best guess, attempting to enter approximately 5 minor
   * ticks between each major tick.
   *
   * <p>If user settings dictate minor ticks to become too dense, they don't make sense, and will be
   * ignored to prevent performance problems.</a>
   *
   * <p>On axes using <a href="#xAxis.categories">categories</a>, minor ticks are not supported.
   */
  ...
  /**
   * The position of the minor tick marks relative to the axis line. Can be one of <code>inside
   * </code> and <code>outside</code>.
   *
   * <p>Defaults to: outside
   */
   ...
   * more axes are displayed on the same side of the plot. With multiple axes the offset is
   ...
   * for vertical axes and bottom for horizontal, so the opposite sides will be right and top
   * respectively. This is typically used with dual or multiple axes.
   *
   * <p>Defaults to: false
   */
  ...
  /**
   * Whether to reverse the axis so that the highest number is closest to the origin. If the chart
   * is inverted, the x axis is reversed by default.
   *
   * <p>Defaults to: false
   */
  ...
  /**
   * If <code>true</code>, the first series in a stack will be drawn on top in a positive,
   * non-reversed Y axis. If <code>false</code>, the first series is in the base of the stack.
   *
   * <p>Defaults to: true
   */
  ...
  /**
   * The amount of ticks to draw on the axis. This opens up for aligning the ticks of multiple
   * charts or panes within a chart. This option overrides the <code>tickPixelInterval</code>
   * option.
   *
   * <p>This option only has an effect on linear axes. Datetime, logarithmic or category axes are
   * not affected.
   */
  ...
  /**
   * The interval of the tick marks in axis units. When <code>null</code>, the tick interval is
   * computed to approximately follow the <a href="#xAxis.tickPixelInterval">tickPixelInterval</a>
   * on linear and datetime axes. On categorized axes, a <code>null</code> tickInterval will default
   * to 1, one category. Note that datetime axes are based on milliseconds, so for example an
   * interval of one day is expressed as <code>24 * 3600 * 1000</code>.
   *
   * <p>On logarithmic axes, the tickInterval is based on powers, so a tickInterval of 1 means one
   ...
   *
   * <p>If the tickInterval is too dense for labels to be drawn, Highcharts may remove ticks.
   *
   * <p>If the chart has multiple axes, the <a href="#chart.alignTicks">alignTicks</a> option may
   * interfere with the <code>tickInterval</code> setting.
   */
   ...
   * tick marks. Not applicable to categorized axis. Defaults to <code>72</code> for the Y axis and
  ...
  /**
   * The position of the major tick marks relative to the axis line. Can be one of <code>inside
   * </code> and <code>outside</code>.
   *
   * <p>Defaults to: outside
   */
   ...
   * behaviour of <a href="#xAxis.tickPixelInterval">tickPixelInterval</a> and <a
  ...
  /**
   * For categorized axes only. If <code>on</code> the tick mark is placed in the center of the
   * category, if <code>between</code> the tick mark is placed between categories. The default is
   * <code>between</code> if the <code>tickInterval</code> is 1, else <code>on</code>.
   *
   * <p>Defaults to: null
   */
  ...
  /**
   * The type of axis. Can be one of <code>linear</code>, <code>logarithmic</code>, <code>datetime
   * </code> or <code>category</code>. In a datetime axis, the numbers are given in milliseconds,
   * and tick marks are placed on appropriate values like full hours or days. In a category axis,
   * the <a href="#series<line>.data.name">point names</a> of the chart's series are used for
   * categories, if not a <a href="#xAxis.categories">categories</a> array is defined.
   *
   * <p>Defaults to: linear
   */
  ...
  /**
   * Sets the height using String presentation. String presentation is similar to what is used in
   * Cascading Style Sheets. Size can be pixels or percentage, otherwise IllegalArgumentException is
   * thrown. The empty string ("") or null will unset the height and set the units to pixels.
   *
   * @param height CSS style string representation
   */
  ...
  /**
   * Sets the height using Vaadin Unit. Only Unit.PIXELS and Unit.PERCENTAGE are supported. In all
   * other cases, IllegalArgumentException is thrown.
   *
   * @param height
  ...
  /**
   * In an ordinal axis, the points are equally spaced in the chart regardless of the actual time or
   * x distance between them. This means that missing data for nights or weekends will not take up
   * space in the chart.
   *
   * <p>Defaults to: true
   */
  ...
  /**
   * The zoomed range to display when only defining one or none of <code>min</code> or <code>max
   * </code>. For example, to show the latest month, a range of one month can be set.
   *
  ...
  /**
   * An optional scrollbar to display on the Y axis in response to limiting the minimum an maximum
   * of the axis values.
   */
  ...
  /**
   * Sets the top using String presentation. String presentation is similar to what is used in
   * Cascading Style Sheets. Size can be pixels or percentage, otherwise IllegalArgumentException is
   * thrown. The empty string ("") or null will unset the height and set the units to pixels.
   *
   * @param top CSS style string representation
   */
  ...
  /**
   * Sets the top using Vaadin Unit. Only Unit.PIXELS and Unit.PERCENTAGE are supported. In all
   * other cases, IllegalArgumentException is thrown.
   *
   * @param top
   * @param unit the unit used for the top
   */
