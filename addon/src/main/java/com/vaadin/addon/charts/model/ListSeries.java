...
/**
 * A series consisting of a list of numerical values. In this case, the numerical values will be
 * interpreted as Y values, and X values will be automatically calculated, either starting at 0 and
 * incrementing by 1, or from pointStart and pointInterval given in the plotOptions. If the axis has
 * categories, these will be used. This option is not available for range series.
 */
public class ListSeries extends AbstractSeries {
  ...
  /**
   * Adds a given number to the series and optionally immediately updates the chart if it has been
   * drawn.
   *
   * <p>This method is useful if you want to add many items without a client/server round-trip for
   * each item added. Do this by specifying false for the updateChartImmediately parameter.
   *
   * @param number the number to be added to the series
   * @param updateChartImmediately if true the chart will be dynamically updated, using animation if
   *     enabled.
  ...
  public void addData(Number number, boolean updateChartImmediately, boolean shift) {
    ...
    if (updateChartImmediately && getConfiguration() != null) {
      getConfiguration().fireDataAdded(this, new DataSeriesItem(data.size() - 1, number), shift);
    }
  }

  /**
   * Updates the value of the data point at pointIndex to newValue and immediately updates it on the
   * chart using animation if enabled.
   *
   * @param pointIndex the index of the point to update
   * @param newValue the new value of the point
   */
