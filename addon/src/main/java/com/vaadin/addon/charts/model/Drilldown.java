...
/**
 * Options for drill down, the concept of inspecting increasingly high resolution data through
 * clicking on chart items like columns or pie slices.
 */
public class Drilldown extends AbstractConfigurationObject {
  ...
  /**
   * Adds a series configurations for the drilldown. These drilldown series are hidden by default.
   * The drilldown series is linked to the parent series' point by its {@link Series#getId()}
   *
   * @param series
   */
  ...
  /**
   * Set the animation for all drilldown animations. Animation of a drilldown occurs when drilling
   * between a column point and a column series, or a pie slice and a full pie series. Drilldown can
   * still be used between series and points of different types, but animation will not occur.
   *
   * @param animation
   */
  ...
  /**
   * Options for the drill up button that appears when drilling down on a series. The text for the
   * button is defined in {@link Lang#setDrillUpText(String)}.
   *
   * @param drillUpButton
   */
  ...
  /**
   * When this option is false, clicking a single point will drill down all points in the same
   * category, equivalent to clicking the X axis label.
   *
   * <p>Defaults to: true
   */
