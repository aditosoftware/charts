...
public class TestClassesSerializable extends TestCase {
  ...
  /**
   * Tests that all the relevant classes and interfaces under {@link #BASE_PACKAGES} implement
   * Serializable.
   *
   * @throws Exception
   */
   ...
   *
   * <p>Adapted from ClassPathExplorer.getRawClasspathEntries(), but without filtering.
   *
  ...
  /**
   * Finds the server side classes/interfaces under a class path entry - either a directory or a JAR
   * that matches {@link #JAR_PATTERN}.
   *
   * <p>Only classes under {@link #BASE_PACKAGES} are considered, and those matching {@link
   * #EXCLUDED_PATTERNS} are filtered out.
   *
   ...
   *
   * @param parentPackage parent package name or null at root of hierarchy, used by recursion
  ...
  private static final Collection<String> findClassesInDirectory(
    ...
    for (File child : files) {
      if (child.isDirectory()) {
        ...
        classNames.add(
            parentPackage.replace(File.separatorChar, '.')
