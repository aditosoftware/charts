...
@SkipFromDemo
public class ColumnWithNativeDrilldownMultipleSeries extends AbstractVaadinChartExample {
  ...
  private void createSeriesAndDrilldown(Configuration conf, int index) {
    ...
    ys =
        new Number[] {
          0.12, 0.19 + index, 0.12, 0.36 + index, 0.32, 9.91 + index, 0.50, 0.22 + index
        };
    drillSeries.setData(categories, ys);
    series.addItemWithDrilldown(item, drillSeries);
    ...
    ys = new Number[] {4.55 + index, 1.42 + index, 0.23, 0.21, 0.20, 0.19 + index, 0.14 + index};
    drillSeries.setData(categories, ys);
    series.addItemWithDrilldown(item, drillSeries);
