...
/**
 * This class consumes the HelMet REST service, which provides data on books in libraries in the
 * Helsinki area.
 */
public class Helmet {
  ...
  public static Result search(String query, int page) throws IOException {
    URL url = null;
    if (page > 0) {
      url = new URL("http://data.kirjastot.fi/search/author.json?query=" + query + "&page=" + page);
    } else {
      url = new URL("http://data.kirjastot.fi/search/author.json?query=" + query);
    }
