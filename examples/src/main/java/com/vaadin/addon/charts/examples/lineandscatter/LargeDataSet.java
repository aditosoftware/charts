...
public class LargeDataSet extends AbstractVaadinChartExample {
   ...
   *
   * <p>Using more datapoints than there are pixels on the devices makes usually no sense. Excess
   * data just consumes bandwidth and client side CPU. This method reduces a large dataset with a
   * very simple method. If you scale up data set by modifying the ROUNDS in this example and toggle
   * passing it through this method, you'll see that visually the chart don't change much, but the
   * amount of data and with huge amounts also the rendering time drops dramatically.
   *
   ...
   * @param points
   * @param epsilon
   * @param xyRatio y values are multiplied with this to make distance calculation in algorithm sane
   * @return
   */
  public static List<DataSeriesItem> ramerDouglasPeucker(
    ...
    if (dmax >= epsilon) {
      ...
      List<DataSeriesItem> startToRelevant =
          ramerDouglasPeucker(points.subList(0, index + 1), epsilon, xyRatio);
      reduced.addAll(startToRelevant);
      List<DataSeriesItem> relevantToEnd =
          ramerDouglasPeucker(points.subList(index, points.size() - 1), epsilon, xyRatio);
  ...
  public static double pointToLineDistance(
      DataSeriesItem A, DataSeriesItem B, DataSeriesItem P, final double xyRatio) {
    ...
    return Math.abs(
            (P.getX().doubleValue() - A.getX().doubleValue()) * (bY - aY)
                - (pY - aY) * (B.getX().doubleValue() - A.getX().doubleValue()))
        / normalLength;
  }
}
