...
public class PieLegendItemClickMouseDetailsTBTest extends AbstractParallelTest {
  ...
  public void testLegendPointIndexMouseDetails() {
    ...
    Assert.assertTrue(
        "Absolute X value wasn't in the expected 200px range", text.contains("AbsX: 2"));
    Assert.assertTrue(
        "Absolute Y value wasn't in the expected 400px range", text.contains("AbsY: 4"));

    Assert.assertTrue(
        "Relative X value wasn't in the expected 100px range", text.contains("RelX: 1"));
    Assert.assertTrue(
        "Relative Y value wasn't in the expected 300px range", text.contains("RelY: 3"));

    Assert.assertTrue(
        "The mouse details element did not contain the meta information from the last click: was:"
            + text,
        text.contains("Bttn: left Alt: false Ctrl: false Meta: false Shift: false"));
  ...
  private void validateMouseDetailsText(
      WebElement legendItemToClick,
    ...
    Assert.assertEquals("Mouse details did not contain expected meta data! ", expected, actual);
  }

  private WebElement getMouseDetails() {
    return driver.findElement(By.id(PieWithLegendAndMouseDetails.MOUSE_DETAILS));
  }
