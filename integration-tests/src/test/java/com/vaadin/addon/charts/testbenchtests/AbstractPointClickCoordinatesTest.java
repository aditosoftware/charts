...
public abstract class AbstractPointClickCoordinatesTest extends AbstractParallelTest {
  ...
  protected void assertPointClickEventCoordinates() {
    ...
    Assert.assertNotNull("No PointClickEvent when there should have been.", eventHistory);
    String[] eventTokens = eventHistory.split(" ");
    Assert.assertEquals("Unexpected contents: " + eventHistory, 6, eventTokens.length);
    Assert.assertEquals("Unexpected event,", "PointClickEvent:", eventTokens[0]);
    Assert.assertEquals(
        "PointClickEvent absoluteX got unexpected value,",
        String.valueOf(expectedPointX),
        eventTokens[2]);
    Assert.assertEquals(
        "PointClickEvent absoluteY got unexpected value,",
        String.valueOf(expectedPointY),
        eventTokens[5]);
  }
  ...
  protected void assertChartClickEventCoordinates() {
    LabelElement lastEvent = $(LabelElement.class).get(1);
    String eventHistory = lastEvent.getText();
    Assert.assertNotNull("No ChartClickEvent when there should have been.", eventHistory);
    String[] eventTokens = eventHistory.split(" ");
    Assert.assertEquals("Unexpected contents: " + eventHistory, 6, eventTokens.length);
    Assert.assertEquals("Unexpected event,", "ChartClickEvent:", eventTokens[0]);
    WebElement chart = findElement(By.className("vaadin-chart"));
    Assert.assertEquals(
        "ChartClickEvent absoluteX got unexpected value,",
        String.valueOf(chart.getLocation().getX() + expectedChartX),
        eventTokens[2]);
    Assert.assertEquals(
        "ChartClickEvent absoluteY got unexpected value,",
  ...
  protected void clickPoint() {
    ...
    new Actions(driver)
        .moveToElement(layout, expectedPointX, expectedPointY)
        .click()
        .build()
        .perform();
    waitForVaadin();
  ...
  private void clickChart() {
    WebElement chart = findElement(By.className("vaadin-chart"));
    new Actions(driver)
        .moveToElement(chart, expectedChartX, expectedChartY)
        .click()
        .build()
        .perform();
    waitForVaadin();
