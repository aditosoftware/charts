...
public class ServerSideEventsTBTest extends AbstractParallelTest {
  ...
  public void chartClick_occured_eventIsFired() {
    ...
    new Actions(driver).moveToElement(chart, 200, 200).click().build().perform();
  ...
  @Test
  public void
      legendItemClick_occuredWhileVisibilityTogglingEnabled_eventAndSeriesHideEventAreFired() {
    openTestUI();
    WebElement legendItem = findLegendItem();
  ...
  public void setXAxesExtremes_occured_eventIsFired() {
    skipBrowser(
        "Selecting area through Testbench does not work correctly",
        Browser.CHROME,
        Browser.PHANTOMJS);
    openTestUI();
    WebElement chart = driver.findElement(By.className("vaadin-chart"));
  ...
  private void assertLastEventIsType(Class<? extends Component.Event> expectedEvent) {
    LabelElement lastEvent = $(LabelElement.class).id("lastEvent");
    Assert.assertEquals(expectedEvent.getSimpleName(), lastEvent.getText());
  }

  private void assertFirstHistoryEventIsType(Class<? extends Component.Event> expectedEvent) {
  ...
  private CheckboxClickEvent readCheckboxEventDetails() {
    ...
    Gson gson =
        new GsonBuilder().registerTypeAdapter(Series.class, new DataSeriesDeserializer()).create();
  ...
  private WebElement findSecondCheckbox() {
    return driver.findElements(By.cssSelector(".vaadin-chart > input[type='checkbox']")).get(1);
  }

  private WebElement findCheckBox() {
    return driver.findElement(By.cssSelector(".vaadin-chart > input[type='checkbox']"));
  }

  private WebElement findDisableVisibityToggle() {
    return driver.findElement(By.cssSelector(".v-checkbox > input[type='checkbox']"));
  }
  ...
  private static class DataSeriesDeserializer implements JsonDeserializer<Series> {
    @Override
    public Series deserialize(JsonElement series, Type type, JsonDeserializationContext jdc)
