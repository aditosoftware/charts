...
public abstract class AbstractParallelTest extends ParallelTest {
  ...
  @Rule public RetryRule maxAttempts = new RetryRule(2);
  ...
  public void setup() throws Exception {
    ...
    if (getBooleanProperty("localPhantom")) {
      WebDriver driver = new SetupDriver().setupLocalDriver(Browser.PHANTOMJS);
      setDriver(driver);
    } else {
      super.setup();
    }
  ...
  /**
   * Waits until the chart element has been rendered on screen. This is necessary in the cases where
   * the test grid is overloaded.
   */
  ...
  protected void waitForElementPresent(By locator, int timeout) {
    waitUntil(ExpectedConditions.presenceOfElementLocated(locator), timeout);
  }
  ...
  /**
   * Tries to automatically determine the IP address of the machine the test is running on.
   *
   * @return An IP address of one of the network interfaces in the machine.
   * @throws RuntimeException if there was an error or no IP was found
   */
  ...
  /**
   * Compares current screen using the TestViewName as base for screenshot reference file
   *
   * @throws IOException
   */
  ...
  /**
   * Compares current screen using the TestViewName and the suffix as base for screenshot reference
   * file
   *
   * @param suffix used in reference file name
   * @throws IOException
   */
  ...
  /**
   * This method is just a workaround until the waitForVaadin() would work also when a chart is
   * dynamically changed. Currently, it only works for initial rendering of the chart.
   */
  ...
  /**
   * Call this method if you want to skip the test on some specific browser. For example, some
   * versions of PhantomJS does not fire onContextMenu event on right click so that browser could be
   * skipped for a test which relays on it.
   *
